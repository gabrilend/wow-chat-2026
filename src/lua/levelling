-- this script awards a player a talent point for every 1/3rd of a level they
-- gain. It stores the total talent points in the database and updates it on
-- levelup or when gaining a new talent point. It also assumes that the talent
-- rate is set to 3 in the worldserver.conf file.

-- Experience = {} -- table holding exp requirements for each level {{{
Experience = {}
Experience[1]  = 400
Experience[2]  = 900
Experience[3]  = 1400
...
TP_PER_LEVEL   = 3 -- if changed then update getExtraTalentPoints() with more tiers
TP_AT_LEVEL_10 = 10
...
function Experience.getTotalTalentPoints(level, currentExp) -- {{{
    -- why 20? Well, 30 is the number of talent points you get from level 1 to 10
    -- and I wanted the players to start with 10 talent points at level 10
...
function Experience.checkLevelProgress(_event, player) -- {{{
    local  totalTalentPoints  = Experience.getTotalTalentPoints(player:GetLevel(), player:GetXP()) -- calculated
    local currentTalentPoints = player:GetData("current-talent-points")                            -- from db
...
function Experience.updateTalentPointDB(playerID, points) -- {{{
    local query = string.format("UPDATE characters SET total_talent_points = %s WHERE guid = %s", points, playerID)
...
function Experience.awardTalentPoint(player, num) -- {{{
    ...
    Experience.updateTalentPointDB(player:GetGUID(), currentTalentPoints + num)
end -- }}}
    ...
    if query then
        ...
        local    player    = GetPlayerByGUID(playerID)
              ...
              if player:GetFreeTalentPoints() > 0 then
                 player:SetFreeTalentPoints(0) end
              end
    else print("player query not found, cannot update talent point data")
    end
end -- }}}

function Experience.onLogin(event, player) -- {{{
    local query = string.format("SELECT guid, total_talent_points FROM characters WHERE guid = %s", player:GetGUID())
