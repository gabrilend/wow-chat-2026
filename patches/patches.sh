...
declare -A PHASE_END_PATCHES=(
    ...
    ["basic"]="E001 E004 E006 E008 E019 E020 E022 E023 E024 E025 E026 E027 E028 E029 E030 E031 E032"  # issue 155: +E032 (2026-09-24, 155k: Outland world monsters +4, the Isle +6). +E031 (2026-09-24, 155k/155f: Outland gear rescaled onto the level-60 ladder; after E024, whose template list it reads). +E030 (2026-09-24, 155o: Darkmoon items never bind; Burning Crusade cards from Tempest Keep and Magisters' Terrace bosses). +E029 (2026-09-24, 155o: Inscription without glyphs; after E027). +E028 (2026-09-24, 155l: Outland without quests, no Shattrath portal to the Isle). +E027 (2026-09-24, 155n: Outland without tradeskills). +E026 (2026-09-24, 155i: the per-creature damage/healing/health multiplier table read by basic_rules.cpp). (2026-09-23): +E025 (155e: a Visiting Mentor per valley for the classes it lacks). +E024 (155f: Outland dungeons a level 60 can enter at level 64; open world stock). +E023 (155d rotation-state table, characters DB). Lua link (src/lua-basic/) + logs + configs + flight-path removal (E008) + no intro cinematic (E019) + Classic riding gates 40/60 (E020) + quests open to the whole faction (E022, 155e). Left out on purpose: E007 level-20 spawn towns (basic spawns in the level-1 valleys), E009/E018 starter kit, E010 pretrained abilities, E021 starting professions — all head-start.
)
# }}}
...
patch_needs_applying_B032() {
    # Witness: Spell.cpp gains the B032 marker around the buff target level check.
    local FILE="${AC_CODE_DIR}/src/server/game/Spells/Spell.cpp"
    [[ -f "${FILE}" ]] && ! grep -q "B032-no-buff-level-restriction" "${FILE}"
}
