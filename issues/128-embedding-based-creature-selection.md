...
## Dependencies
- Issue 127: contextual-creature-spawns (type filtering)
...
```lua
function Ambush.selectDiverseCreature(candidates, recentSpawns)
    local maxDistance = 0
    local selected = nil
    ...
    for _, candidate in ipairs(candidates) do
        ...
        for _, recent in ipairs(recentSpawns) do
            ...
            local dist = cosineSimilarity(candidateEmb, recentEmb)
            -- We want DISSIMILARITY, so use 1 - similarity
            local dissim = 1 - dist
        ...
        -- (most different from the most similar recent spawn)
        if minDistToRecent > maxDistance then
            maxDistance = minDistToRecent
            selected = candidate
        end
    end
...
## Alternative: Zone Context Matching

Instead of (or in addition to) diversity from recent spawns,
match creature names to zone context:

```lua
-- Zone has semantic embedding too
ZoneEmbeddings = {
    ["Tirisfal Glades"] = { ... },
    ["Deadmines"]       = { ... },
}

-- Select creature whose name FITS the zone best
-- "Haunted Servitor" fits "graveyard" better than "Feral Ghoul"
```

This is the opposite approach: similarity to context vs diversity from history.

Could combine both:
1. Filter by type
2. Rank by zone context similarity
3. Among top matches, pick most different from recent

## Performance Considerations

### Memory
- 10,000 creatures × 384 floats × 4 bytes = 15 MB
- Reduced to 64 dims: 2.5 MB
- Acceptable for server memory

### CPU
- Cosine similarity: O(embedding_dim) per comparison
- 100 candidates × 5 recent × 64 dims = 32,000 operations
...
This approach is inspired by the neocities-modernization project's
similarity-engine.lua, which implements cosine similarity for
embedding comparison. Key patterns borrowed:

1. **Cosine Similarity**: Standard dot product / (norm_a × norm_b)
2. **Precomputed Embeddings**: Load at startup, O(1) lookup
3. **Configurable Selection**: Rather than hardcoded "most different",
   allow parameterization for different gameplay contexts

See: `/home/ritz/programming/ai-stuff/neocities-modernization/src/similarity-engine.lua`
...
- issues/127-contextual-creature-spawns.md (type filtering)
