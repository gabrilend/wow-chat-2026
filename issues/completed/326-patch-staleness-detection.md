...
**scripts/azerothcore:**
- Added `patches_need_applying()` function (lines 575-632)
- Checks each PHASE_BEGIN patch (B001-B008) to see if it would modify source
- Returns 0 (true) if any patch needs applying, 1 (false) if all applied
- Integration already present at line 1492 in `cmd_update`

**Patch Detection Logic:**

Each patch has an idempotent check:
- **B001**: Checks for old `Item* pItem = player->GetItemByGuid` pattern
- **B002**: Looks for `sALE->OnLogin(bot)` call
- **B003**: Looks for gameobject wildcard check
- **B004**: Checks if warning fix applied to BudgetValues.cpp
- **B005**: Looks for `#define ACCURACY_LEVEL_CAP 3`
- **B006**: Looks for `PLAYER_EVENT_ON_SELL_ITEM` hook
- **B007**: Looks for `int SetWalk` in UnitMethods.h
- **B008**: Checks if mod-talent-bonus module linked

### How It Works

```bash
# Before (lines 1524-1531):
if [[ "${needs_rebuild}" == false && -f "${INSTALL_DIR}/bin/worldserver" ]]; then
    echo "Everything up-to-date, no rebuild needed."
    return 0  # EXIT - patches might not be applied!
fi

# After (lines 1490-1531):
# Check if patches need applying
if patches_need_applying; then
    echo "Patches pending - rebuild required"
    needs_rebuild=true
...
$ echo $?
0  # Patches need applying (return 0 = true in bash)
```

Function correctly detects when patches need to be applied and prevents early exit.

### Maintenance Note

**When adding new patches (B009, B010, etc.):**
1. Add patch function to `patches/*.sh`
2. Add idempotent check to `patches_need_applying()` function
3. This ensures staleness detection stays current with patch registry

### Benefits Achieved

1. **No more missed patches** - New patches always applied even if upstream unchanged
2. **Fast when nothing changed** - Only greps a few files, very quick
3. **Self-documenting** - Each check explains what it's looking for
4. **Easy to extend** - Adding new patch = adding new check
5. **Works with timestamps** - Dual detection (staleness + timestamp)
...
- Issue 334 - Patch system improvements (foundation for this feature)
