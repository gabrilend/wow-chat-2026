...
- Phase: 2
- Priority: N/A
- **Superseded by:** 405-profile-transition-system, 406-alpha-baseline-setup
...
This proposal to remove the profile system has been superseded by a refined profile implementation:

**What was implemented instead (issues 405, 406):**
- Two source directories: `source-alpha/` and `source-beta/`
- Three profiles: alpha (wow-chat-1 baseline), release (bare playerbots), beta (experimental)
- File-based profile distinction (*.alpha.lua, *.beta.lua, *.release.lua)
- Module symlink system (modules-beta vs modules-release)

**Why the profile system is valuable:**
1. **Alpha = regression baseline** - Known-good fallback when beta breaks
2. **Release = production-ready** - Bare playerbots, no experimental features
3. **Beta = development space** - Lawless experimentation without risking stability
4. **Two sources, not three** - Alpha uses different codebase (old Eluna)

**Key difference from this proposal:**
This issue proposed using git branches for feature isolation. Instead, we use profiles with file suffixes and module directories. Git branches track the overall project state, profiles control feature sets within a build.

The profile system adds value by providing multiple stable states without branch switching overhead. Alpha must always work (regression safety), release must be deployable (user-facing), beta can break (development freedom).

**Status:** Keeping profile system as implemented in 405/406.
